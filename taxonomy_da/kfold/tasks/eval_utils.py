    ...
    with torch.no_grad():
        ...
        for _, batch in enumerate(dataloader):
            ...
            if args.pretrained_bert:
                tokens, types, attention_mask = data['text'], data['types'], data['padding_mask']
                inputs = [tokens, types, attention_mask]
            elif args.t5_model:
                input_ids, attention_mask = data["text"], data["padding_mask"]
                dec_input_ids, dec_target_ids, dec_logit_mask = data["dec_text"], data["dec_target"], data[
                    "dec_logit_mask"]
                inputs = [input_ids, attention_mask, dec_input_ids, dec_target_ids, dec_logit_mask]
            elif args.cloze_eval:
                tokens, position_ids = data['text'], data['position']
                ...
                tokens, position_ids, attention_mask = data['text'], data['position'], data['mask']
            ...
            elif (args.cloze_eval and args.fast_decode) or args.t5_model:
                num_enc_args = 2 if args.t5_model else 3
                logit_list = []
                num_choices = inputs[num_enc_args].size(1)
                for i in range((num_choices - 1) // segment_length + 1):
                    input_batch = inputs[:num_choices] + [arg[:, i * segment_length: (i + 1) * segment_length] for arg in
                                                inputs[num_enc_args:]]
                    logits = model(*input_batch)
                    logit_list.append(logits)
                logits = torch.cat(logit_list, dim=1)

...
def build_input_from_ids(text_a_ids, text_b_ids, answer_ids, max_seq_length, tokenizer, args=None, add_cls=True,
    ...
    return {"text": ids, "types": types, "padding_mask": paddings, "position": position_ids, "mask": sep,
            "target": target_ids, "logit_mask": loss_masks}


def build_decoder_input(enc_ids, answer_ids, max_seq_length, max_dec_seq_length, tokenizer, mask_id=None):
    if mask_id is None:
        mask_id = tokenizer.get_command('MASK').Id
    eos_id = tokenizer.get_command('eos').Id
    sop_id = tokenizer.get_command('sop').Id
    ...
    return {"dec_text": ids, "dec_types": types, "dec_padding_mask": paddings, "dec_position": position_ids,
            "dec_mask": masks, "dec_target": target_ids, "dec_logit_mask": loss_masks}

...
class Template(yaml.YAMLObject):
    ...
    def apply_revise(self, example, source_task_name, truncate=True, highlight_variables=False):
        """
        ...
        # Zongyu: use the source answer choices
        prompts = DatasetTemplates(source_task_name)
        # 任选一个 -> 选一个answer choices最多的
        if source_task_name == "trec":
            src_jinja = prompts["trec1"]
        else:
            pns = prompts.all_template_names
            for pn in pns:
                src_jinja = prompts[pn]
                if src_jinja.answer_choices != None:
                    break
        ...
        # protected_example = self._escape_pipe(example)
        # rendered_choices = rtemplate.render(**protected_example)
        # answer_choices = [self._unescape_pipe(answer_choice.strip()) for answer_choice in rendered_choices.split("|||")]
        hhh = src_jinja.answer_choices
        protected_example["answer_choices"] = hhh.split("|||")

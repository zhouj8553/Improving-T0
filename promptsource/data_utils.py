...
TASK_TYPE_DICT = {
    ...
    "extractive_qa":[
        ...
        "adversarial_qa/droberta",
        "duorc/SelfRC",
        "duorc/Paraphrase_IdentificationRC",
        "ropes",
        "squad_v2",
        "super_glue/record",
        "quoref",
...
def _apply_t0_prompts(task_name, dataset_split):
    def prompt_input_target(example):
        ...
        if len(results) == 1 and len(results[0]) == 0: # some label will lead to None, e.g., if two sentences are not similar to each other (glue/mrpc)

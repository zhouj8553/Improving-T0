...
class DataCollatorForMultipleChoice:
    ...
    def __call__(self, features):
        ...
        # num_choices=[len(f['input_ids']) for f in features]
        flattened_features = [
            [
                {
                    k: v[i]
                    for k, v in feature.items()
                    if (k != "targets" and k != "answer_choices_ids" and k != "answer_choices_texts")
                }
        ...
        if 'labels' not in flattened_features[0]: 
            batch = {
                k: torch.tensor(v)
        ...
        if 'answer_choices_ids' in features[0]: 
            return batch

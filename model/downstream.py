...
class T5ForCloze(torch.nn.Module):
    def __init__(self, args):
        super().__init__()
        self.model = T5Model(args)
        self.take_softmax = True
        self.length_penalty = args.length_penalty
    ...
    def forward(self, enc_input_ids, enc_attention_mask, dec_input_ids, dec_target_ids, dec_logit_mask):
        # Encode
        enc_attention_mask = enc_attention_mask.unsqueeze(1).unsqueeze(1)
        batch_size, seq_length = enc_input_ids.size()[:2]
        encoder_outputs = self.model.encode(enc_input_ids, enc_attention_mask)

        # Reshape decoder inputs
        hidden_size = encoder_outputs.size()[-1]
        num_choices, tgt_seq_length = dec_input_ids.size()[1:]
        ...
        encoder_outputs = encoder_outputs.reshape(batch_size * num_choices, seq_length, hidden_size)
        enc_attention_mask = enc_attention_mask.reshape(batch_size * num_choices, 1, 1, seq_length)
        dec_input_ids = dec_input_ids.reshape(-1, tgt_seq_length)
        outputs, *_ = self.model.decode(dec_input_ids, None, encoder_outputs=encoder_outputs,

...
repeat_task_names=['cosmos_qa','social_i_qa','paws/labeled_final','quail','wiki_qa','quartz','qasc','ropes']
...
for i,task_name in enumerate(task_names):
    if task_name in repeat_task_names: continue
    print(i,task_name)
    np.random.seed(42)
    random.seed(42)
    ...
    if task_name not in repeat_task_names:
...
import pdb 
pdb.set_trace()
train_data.save_to_disk(os.path.join(save_dir,'only_posthoc_wo_top8_{}_0616/train'.format(large_num)))
...
valid_data.save_to_disk(os.path.join(save_dir,'only_posthoc_wo_top8_{}_0616/validation'.format(large_num)))
import pdb 
pdb.set_trace()

# python -m universal_da.sample_data.prepare_wo_top8

...
for i,task_name in enumerate(task_names):
    ...
    if task_name in ['glue/qqp','wiqa','wiki_hop/original','trec','wiki_bio']:
        ...
        train_examples,valid_dataset=T0_data_utils.load_datasets(data_dir=multi_data_dir,task_name=task_name,train_num=50000,valid_num=max_valid_num_per_dataset,k_fold=-1,fold_id=-1)

        tmp_train_data=datasets.load_from_disk(os.path.join('../../spec_datasets/details_CrossOrigindataset_crossed_correct_balanced_0523',task_name))
        print(len(train_examples))
        if len(train_examples)>=50000:
            all_num=len(train_examples)
            idxs=list(range(len(tmp_train_data)))
            num=50000
            if len(idxs)>num:
                sample_idxs=np.random.choice(idxs,num,False).tolist()
                tmp_train_data=tmp_train_data.select(sample_idxs)
            else:
                print('pass')
        else:
            print('no action')
        ...
        if task_name in repeat_task_names:
            for _ in range(repeat_times):
                print('repeat {}'.format(task_name))
                total_train_examples.append(tmp_train_data)
        else:
            total_train_examples.append(tmp_train_data)
...
train_data.save_to_disk(os.path.join(save_dir,'support_p2_upsample_{}_0610_800w/train'.format(repeat_times)))
...
valid_data.save_to_disk(os.path.join(save_dir,'support_p2_upsample_{}_0610_800w/validation'.format(repeat_times)))
import pdb 
pdb.set_trace()

# python -m universal_da.sample_data.prepare_upsample_cross

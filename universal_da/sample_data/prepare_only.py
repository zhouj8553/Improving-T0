...
for i,task_name in enumerate(task_names):
    if task_name not in repeat_task_names: continue
    print(i,task_name)
    np.random.seed(42)
    random.seed(42)
...
import pdb 
pdb.set_trace()
train_data.save_to_disk(os.path.join(save_dir,'only_posthoc_top8_{}_0616/train'.format(large_num)))
...
valid_data.save_to_disk(os.path.join(save_dir,'only_posthoc_top8_{}_0616/validation'.format(large_num)))
import pdb 
pdb.set_trace()

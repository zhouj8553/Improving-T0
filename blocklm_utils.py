...
class CollatorForT5MLM:
    def __init__(self, tokenizer, noise_density, mean_noise_span_length, prefix_lm=False):
    ...
    def __call__(self, examples: List[Dict[str, np.ndarray]]) -> Dict[str, np.ndarray]:
        ...
        if self.prefix_lm:
            max_span_length = int(expandend_input_length * self.noise_density)
            span_length = rng.randint(1, max_span_length + 1)
            ...
            labels = np.concatenate(
                [labels, np.full((batch_size, 1), self.tokenizer.get_command('eos').Id, dtype=np.int32)], axis=-1
            )
            text = torch.tensor(input_ids, dtype=torch.long)
            target = torch.tensor(labels, dtype=torch.long)
        else:
            mask_indices = np.asarray([self.random_spans_noise_mask(expandend_input_length, rng) for _ in range(batch_size)])
            labels_mask = ~mask_indices

            input_ids_sentinel = self.create_sentinel_ids(mask_indices.astype(np.int8))
            labels_sentinel = self.create_sentinel_ids(labels_mask.astype(np.int8))

            text = torch.tensor(self.filter_input_ids(input_ids, input_ids_sentinel), dtype=torch.long)
            target = torch.tensor(self.filter_input_ids(input_ids, labels_sentinel), dtype=torch.long)

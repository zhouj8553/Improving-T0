...
class XLDataset(data.Dataset):
    def __init__(self, ds, tokenizer, max_seq_len=1024, mem_len=None, **kwargs):
        self.ds = ds
...
class BlockDataset(data.Dataset):
    def __init__(self, ds, tokenizer,
                 max_seq_len=1024,
                 sample_one_document=True,
                 non_sentence_start=0.0, filter_english=False, add_cls=True, add_eos=True, sentence_end=True, **kwargs):
        """
        ...
        self.max_seq_len = max_seq_len
        self.tokenizer = tokenizer
        self.sample_across_doc = not sample_one_document
    ...
    def get_random_sample(self, np_rng):
        while True:
            idx = np_rng.randint(self.ds_len)
            item = self.ds[idx]
            tokens, loss_mask = item['tokens'], item['loss_masks']
            if self.add_eos:
    ...
    def __getitem__(self, idx):
        ...
        tokens, loss_mask = self.get_random_sample(rng)
        # truncate or pad tokens
        num_tokens = len(tokens)
        tokens_to_strip = num_tokens - self.max_seq_len
        if self.add_cls:
            tokens_to_strip = tokens_to_strip + 1
        ...
        if self.add_eos and tokens[0] == self.tokenizer.get_command('eos').Id:
            tokens, loss_mask = [], []
        elif self.add_cls:
        ...
        if self.sample_across_doc:
            while len(tokens) < self.max_seq_len:
                new_tokens, new_loss_mask = self.get_random_sample(rng)
                if self.add_cls:
    ...
    def right_strip_seq(self, tokens, loss_mask, seq_length):
        ...
        if strip_right_tokens > 0:
            ...
            if not self.sentence_end or len(tokens) - strip_right_tokens < seq_length // 2:
                strip_right_tokens = len(tokens) - seq_length

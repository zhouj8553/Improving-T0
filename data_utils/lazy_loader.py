...
class LazyLoader(object):
    ...
    def __init__(self, path, data_type='data', mem_map=False, map_fn=None, is_array=False, array_data_type=np.int32,
                 load_memory=False, loader_range=None):
        lazypath = get_lazy_path(path)
        ...
        start_index, end_index, offset = None, None, None
        if loader_range:
            start_index, end_index = int(len(self.lens) * loader_range[0]), int(len(self.lens) * loader_range[1])
            offset = sum(self.lens[:start_index])
            self.lens = self.lens[start_index: end_index]
        ...
        if self.load_memory:
            if loader_range:
                self.file.seek(offset * data_type_size)
                self.file = self.file.read(self.ends[-1] * data_type_size)
            else:
            ...
            if is_array:
                if self.ends[-1] == 0:
                    ...
                    offset = offset * data_type_size if loader_range else 0
                    shape = self.ends[-1] if loader_range else None
                ...
                if self.ends[-1] == 0:
                    ...
                    length = self.ends[-1] if loader_range else 0
                    offset = offset if loader_range else 0
                    self.file = mmap.mmap(self.file.fileno(), length, prot=mmap.PROT_READ, offset=offset)
        self.read_lock = Lock()
